...
import type { AdminView, Notification } from "../../types";
...
interface HeaderProps {
  ...
  viewTitlesOverride?: Partial<Record<AdminView, string>>;
}

// Default view title mapping
const defaultViewTitles: Partial<Record<AdminView, string>> = {
  dashboard: "Dashboard",
...
export function Header({
  ...
  const viewTitles = { ...defaultViewTitles, ...viewTitlesOverride };
  ...
  return (
    <div className="absolute right-0 top-full mt-2 w-96 bg-slate-900 border border-slate-800 rounded-lg shadow-2xl z-50 max-h-[500px] overflow-hidden flex flex-col">
      <div className="p-4 border-b border-slate-800 flex items-center justify-between">
        ...
        {unreadCount > 0 && (
          <button onClick={onMarkAllAsRead} className={`text-xs ${accentText} hover:opacity-80`}>
            Mark all as read
          </button>
        )}
      </div>
