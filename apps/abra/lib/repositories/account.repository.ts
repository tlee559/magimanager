...
class AccountRepository {
  ...
  async delete(id: string): Promise<void> {
    ...
    await prisma.accountCheckIn.deleteMany({ where: { adAccountId: id } });
    await prisma.accountActivity.deleteMany({ where: { adAccountId: id } });
    await prisma.dailySpendSnapshot.deleteMany({ where: { adAccountId: id } });
    await prisma.threadMessage.deleteMany({ where: { thread: { adAccountId: id } } });
    await prisma.accountThread.deleteMany({ where: { adAccountId: id } });
    await prisma.dismissedAlert.deleteMany({ where: { adAccountId: id } });
  ...
  async addCheckIn(
    ...
    const checkIn = await prisma.accountCheckIn.create({
      data: {
        adAccountId: accountId,
