...
class IdentityRepository {
  ...
  async count(options: IdentityFindOptions = {}): Promise<number> {
    return prisma.identityProfile.count({
      where: this.buildWhere(options),
    });
  }
  ...
  async delete(id: string): Promise<void> {
    ...
    await prisma.identityDocument.deleteMany({ where: { identityProfileId: id } });
    await prisma.identityActivity.deleteMany({ where: { identityProfileId: id } });
    await prisma.goLoginProfile.deleteMany({ where: { identityProfileId: id } });
  ...
  async removeDocument(documentId: string): Promise<void> {
    await prisma.identityDocument.delete({ where: { id: documentId } });
  }

  async getUniqueGeos(): Promise<string[]> {
    const result = await prisma.identityProfile.findMany({
      select: { geo: true },
      distinct: ["geo"],
      orderBy: { geo: "asc" },
    });
