...
import { accountRepository, identityRepository, type AccountFindOptions, type AccountWithRelations } from "@/lib/repositories";
import type {
...
class AccountService {
  ...
  ): Promise<AccountServiceResult<AccountWithRelations>> {
    try {
  ...
  async getByInternalId(internalId: number): Promise<AccountServiceResult<AccountWithRelations>> {
    try {
  ...
  async getByGoogleCid(googleCid: string): Promise<AccountServiceResult<AccountWithRelations>> {
    try {
  ...
  async getAll(options: AccountFindOptions = {}): Promise<AccountServiceResult<AccountWithRelations[]>> {
    try {
  ...
  async delete(id: string, userId?: string | null): Promise<AccountServiceResult<void>> {
    try {
      const existing = await accountRepository.findById(id);
      if (!existing) {
        return { success: false, error: "Account not found" };
  ...
  async archive(id: string, userId?: string | null): Promise<AccountServiceResult<AccountWithRelations>> {
    try {
      const existing = await accountRepository.findById(id);
      if (!existing) {
        return { success: false, error: "Account not found" };
  ...
  async addCheckIn(
    ...
    userId?: string | null
  ): Promise<AccountServiceResult<CheckIn>> {
    try {
      const existing = await accountRepository.findById(id);
      if (!existing) {
        return { success: false, error: "Account not found" };
  ...
  async logActivity(
    ...
    details: string | null,
    userId?: string | null
  ): Promise<AccountServiceResult<AccountActivity>> {
    try {
      const existing = await accountRepository.findById(id);
      if (!existing) {
        return { success: false, error: "Account not found" };
