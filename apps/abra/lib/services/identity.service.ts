...
import { identityRepository, type IdentityFindOptions, type IdentityWithRelations } from "@/lib/repositories";
...
export interface IdentityServiceResult<T> {
  success: boolean;
  data?: T;
  error?: string;
}
...
class IdentityService {
  async getById(id: string): Promise<IdentityServiceResult<IdentityWithRelations>> {
    try {
  ...
  async getAll(options: IdentityFindOptions = {}): Promise<IdentityServiceResult<IdentityWithRelations[]>> {
    try {
  ...
  async create(
    ...
    userId?: string | null
  ): Promise<IdentityServiceResult<IdentityWithRelations>> {
    try {
  ...
  async delete(id: string, userId?: string | null): Promise<IdentityServiceResult<void>> {
    try {
      // Check if identity exists
      const existing = await identityRepository.findById(id);
      if (!existing) {
  ...
  async archive(id: string, userId?: string | null): Promise<IdentityServiceResult<IdentityWithRelations>> {
    try {
      const existing = await identityRepository.findById(id);
      if (!existing) {
        return { success: false, error: "Identity not found" };
  ...
  async unarchive(id: string, userId?: string | null): Promise<IdentityServiceResult<IdentityWithRelations>> {
    try {
      const existing = await identityRepository.findById(id);
      if (!existing) {
        return { success: false, error: "Identity not found" };
  ...
  async addDocument(
    ...
    filePath: string,
    userId?: string | null
  ): Promise<IdentityServiceResult<IdentityDocument>> {
    try {
  ...
  async removeDocument(
    ...
    documentId: string,
    userId?: string | null
  ): Promise<IdentityServiceResult<void>> {
    try {
      const existing = await identityRepository.findById(identityId);
      if (!existing) {
        return { success: false, error: "Identity not found" };
