 ...
 */
export function encryptOAuthState(data: { cid?: string; accountId?: string; csrf: string; debug?: boolean }): string {
  const stateData = {
    ...data,
    timestamp: Date.now(),
  };
 ...
 */
export function decryptOAuthState(encryptedState: string): { cid?: string; accountId?: string; csrf: string; timestamp: number; debug?: boolean } {
