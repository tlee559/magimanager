...
/**
 * PATCH /api/campaigns/[campaignId]/keywords/[keywordId]/status
 *
 * Update keyword status (pause/enable)
 * Requires: keywords.pause feature flag
 */
export async function PATCH(
  req: NextRequest,
  { params }: { params: Promise<{ campaignId: string; keywordId: string }> }
) {
  try {
    ...
    if (!isFeatureEnabled("keywords.pause")) {
      return NextResponse.json(
        { error: "Keyword status updates are not enabled" },
        { status: 403 }
      );
    }
    ...
    const result = await mutateKeywordStatus(
      accessToken,
      customerId,
      adGroupId,
      keywordId,
    ...
    if (!result.success) {
      return NextResponse.json(
        { error: result.error || "Failed to update keyword status" },
        { status: 500 }
      );
    }
    ...
    await prisma.accountActivity.create({
      data: {
        ...
        action: status === "PAUSED" ? "KEYWORD_PAUSED" : "KEYWORD_ENABLED",
        details: `Keyword ${keywordId} ${status === "PAUSED" ? "paused" : "enabled"} by ${session.user.name || session.user.email}`,
      },
    });
