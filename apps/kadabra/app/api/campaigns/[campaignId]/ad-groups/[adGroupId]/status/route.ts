...
/**
 * PATCH /api/campaigns/[campaignId]/ad-groups/[adGroupId]/status
 *
 * Update ad group status (pause/enable)
 * Requires: campaigns.pause feature flag (ad groups follow campaign permissions)
 */
export async function PATCH(
  req: NextRequest,
  { params }: { params: Promise<{ campaignId: string; adGroupId: string }> }
) {
  try {
    ...
    if (!isFeatureEnabled("campaigns.pause")) {
      return NextResponse.json(
        { error: "Ad group status updates are not enabled" },
        { status: 403 }
      );
    }
    ...
    const { accountId, customerId, status } = body;
    ...
    const result = await mutateAdGroupStatus(
      accessToken,
      customerId,
      adGroupId,
      status,
    ...
    if (!result.success) {
      return NextResponse.json(
        { error: result.error || "Failed to update ad group status" },
        { status: 500 }
      );
    }
    ...
    await prisma.accountActivity.create({
      data: {
        ...
        action: status === "PAUSED" ? "AD_GROUP_PAUSED" : "AD_GROUP_ENABLED",
        details: `Ad group ${adGroupId} ${status === "PAUSED" ? "paused" : "enabled"} by ${session.user.name || session.user.email}`,
      },
    });
