 ...
 * Requires: campaigns.pause feature flag
 */
  ...
  try {
    ...
    if (!isFeatureEnabled("campaigns.pause")) {
      return NextResponse.json(
        { error: "Campaign status updates are not enabled" },
        { status: 403 }
      );
    }
    ...
    await prisma.accountActivity.create({
      data: {
        ...
        action: status === "PAUSED" ? "CAMPAIGN_PAUSED" : "CAMPAIGN_ENABLED",
        details: `Campaign ${campaignId} ${status === "PAUSED" ? "paused" : "enabled"} by ${session.user.name || session.user.email}`,
      },
    });
